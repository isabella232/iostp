...
Rickshaw.Graph.Ajax=Rickshaw.Class.create(
    {
        initialize:function(args){
            ...
            this.args=args;this.request()
        },
        request:function(){
            $.ajax(
                {
                    url:this.dataURL,dataType:"json",
        ...
        _splice:function(args){
            ...
            if(!args.series)return data;series.forEach(function(s){
...
Rickshaw.Graph.Axis.X=function(args){
    ...
    this.render=function(){
        ...
        if(this.orientation=="top"){
            var yOffset=this.height||this.graph.height;transform="translate("+berth+","+yOffset+")"
        }else{
...
Rickshaw.Graph.Axis.Y=Rickshaw.Class.create({
    ...
    _drawGrid:function(axis){
        var gridSize=(this.orientation=="right"?1:-1)*this.graph.width;this.graph.vis.append("svg:g").attr("class","y_grid").call(axis.ticks(this.ticks).tickSubdivide(0).tickSize(gridSize))
    }
});
...
Rickshaw.Graph.HoverDetail=Rickshaw.Class.create(
    {
        initialize:function(args){
            ...
            this.visible=true;graph.element.appendChild(element);
            this.lastEvent=null;
            this._addListeners();
            this.onShow=args.onShow;this.onHide=args.onHide;
            ...
            this.graph.series.active().forEach(function(series){
                ...
                for(var i=approximateIndex;i<data.length-1;){
                    ...
                    if(data[i].x<=domainX&&data[i+1].x>domainX){
                        dataIndex=i;break}if(data[i+1].x<=domainX){i++}
                    else{i--}
                }
        ...
        _addListeners:function(){
            this.graph.element.addEventListener("mousemove",function(e){
                this.visible=true;this.update(e)
            }.bind(this),false);
            this.graph.onUpdate(function(){
...
Rickshaw.Graph.Legend=function(args){
    ...
    var list=this.list=document.createElement("ul");element.appendChild(list);
...
Rickshaw.Graph.Renderer=Rickshaw.Class.create({
    initialize:function(args){
        ...
        this.graph.unstacker=this.graph.unstacker||new Rickshaw.Graph.Unstacker({
            graph:this.graph
        });
        this.configure(args)
    },
    ...
    domain:function(){
        ...
        if(this.graph.max===undefined){
            yMax+=(yMax-yMin)*this.padding.top}return {x:[xMin,xMax],y:[yMin,yMax]}
    },
    render:function(){
        var graph=this.graph;
...
Rickshaw.Graph.Renderer.Line=Rickshaw.Class.create(Rickshaw.Graph.Renderer,{
    ...
    defaults:function($super){
        return Rickshaw.extend($super(),{
            unstack:true,fill:false,stroke:true
        })
    },
    seriesPathFactory:function(){
        var graph=this.graph;
        var factory=d3.svg.line().x(function(d){
            return graph.x(d.x)}).y(function(d){
                return graph.y(d.y)
...
Rickshaw.Graph.RangeSlider = Rickshaw.Class.create({
	...
	build: function() {
        ...
        if( that.graph.constructor === Array ) {
            $( function() {
                $(element).slider( {
                    range: true,
                    min: that.graph[0].dataDomain()[0],
                    max: that.graph[0].dataDomain()[1],
                    values: [
                        that.graph[0].dataDomain()[0],
                        that.graph[0].dataDomain()[1]
                    ],
                    slide: function( event, ui ) {
                        for( var i=0; i<that.graph.length; i++) {
                            that.graph[i].window.xMin = ui.values[0];
                            that.graph[i].window.xMax = ui.values[1];
                            that.graph[i].update();

                            // if we're at an extreme, stick there
                            if (that.graph[i].dataDomain()[0] == ui.values[0]) {
                                that.graph[i].window.xMin = undefined;
                            }
                            if (that.graph[i].dataDomain()[1] == ui.values[1]) {
                                that.graph[i].window.xMax = undefined;
                            }
                        }
                        if( onslide ) {
            ...
            element[0].style.width = this.graph[0].width + 'px';
